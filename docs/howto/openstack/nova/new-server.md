...
=== "OpenStack CLI"
    ...
    Your server should have an image to boot from (`IMAGE_NAME`).
    ...
    You need at least one network in the region where you're creating your new server (`NETWORK_NAME`).
    ...
    You'll most likely want a server you can connect to remotely via SSH without typing a password.
    ...
    When you use it, you get back control of your terminal only after the server is ready.

    The network your server is attached to might be dual-stack, or it might have an IPv4-based subnet.
    If so, and you also want the server to be reachable via a public IPv4 address, create a floating IP and assign it to your server.
...
=== "{{gui}}"
    ...
    Please note that this window cannot be resized, but you can open it in a new browser window or tab.
...
However, we do not recommend using the web console.
